...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Detect embedded in-app browsers (LinkedIn, Instagram, FB, etc.) which Google blocks.
    const userAgent = navigator.userAgent || navigator.vendor || (window as any).opera;
    const isIOSWebView = /(iPhone|iPod|iPad).*AppleWebKit(?!.*Safari)/i.test(userAgent);
    const isAndroidWebView = /Android.*(wv|\.0\.0\.0)/i.test(userAgent);
    const isSocialApp = /FBAV|FBAN|Instagram|LinkedInApp|Snapchat|Twitter|Line|WhatsApp/i.test(userAgent);
    
    if (isIOSWebView || isAndroidWebView || isSocialApp) {
    ...
    <div className="min-h-[125vh] bg-[#5ce1e6] flex flex-col font-sans text-black selection:bg-[#ff90e8]">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-md brutal-border bg-white p-10 relative brutal-shadow-lg transform rotate-1">
          ...
          {isInAppBrowser ? (
            <div className="space-y-4 text-center bg-[#FFD84D] p-6 brutal-border transform rotate-1 brutal-shadow-sm">
              <h3 className="font-black text-2xl uppercase mb-2 text-black">Browser Blocked</h3>
              <p className="font-bold text-black text-sm">
                Google Login is blocked inside this app. Tap the <strong>3 dots (⋮)</strong> in the top right corner and select <strong>"Open in Browser"</strong> or <strong>"Open in Chrome/Safari"</strong> to continue.
              </p>
            </div>
          ) : (
